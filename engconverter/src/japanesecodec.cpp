...
/*!
  \fn JpUnicodeConv::JpUnicodeConv (int r)
  \internal
*/
...
JapaneseCodec::JapaneseCodec() : conv(JpUnicodeConv::newConverter(JpUnicodeConv::Default))
{
}
...
*/
JapaneseCodec::~JapaneseCodec()
{
    delete (JpUnicodeConv*)conv;
    conv = 0;
}

int JapaneseCodec::mibEnum() const
{
    return 17;
}

QByteArray JapaneseCodec::name() const
{
    return "Shift_JIS";
}

QByteArray JapaneseCodec::convertFromUnicode(const QChar *uc, int len, ConverterState *state) const
{
    char replacement = '?';
    if (state) {
...
QString JapaneseCodec::convertToUnicode(const char* chars, int len, ConverterState *state) const
{
    uchar buf[1] = {0};
    int nbuf = 0;
