...
KoreanCodec::KoreanCodec()
{
}

int KoreanCodec::mibEnum() const
{
  return -949; // CP949 has no MIBenum. So we have to use fake value.
}

QByteArray KoreanCodec::name() const
{
  return "cp949";
}
...
/*!
  \reimp
*/
QByteArray KoreanCodec::convertFromUnicode(const QChar *uc, int len, ConverterState *state) const
{
    char replacement = '?';
    if (state) {
...
/*!
  \reimp
*/
QString KoreanCodec::convertToUnicode(const char* chars, int len, ConverterState *state) const
{
    uchar buf[2] = {0, 0};
    int nbuf = 0;
